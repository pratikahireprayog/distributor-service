...
import {
  ...
  XpressbeesB2cAuthResponseDto,
} from './xpressbees_b2c.dto';
import {
  XPRESSBEES_B2C_ENV_KEYS,
  XPRESSBEES_B2C_DEFAULTS,
  XPRESSBEES_B2C_CONSTANTS,
} from './xpressbees_b2c-constants';

@Injectable()
export class XpressbeesB2cAuthService implements AuthProvider {
  private readonly logger = new Logger(XpressbeesB2cAuthService.name);
  ...
  private async refreshToken(): Promise<string> {
    try {
      this.logger.log('Refreshing XpressBees B2C auth token');
      ...
      const authPath = this.configService.get<string>(
        XPRESSBEES_B2C_ENV_KEYS.AUTH_PATH,
        XPRESSBEES_B2C_DEFAULTS.AUTH_PATH
      );
      
      // Hardcoded credentials as fallback (for development only)
      const email = this.configService.get<string>(
        XPRESSBEES_B2C_ENV_KEYS.AUTH_EMAIL,
        'mukesh@cargodham.com'
      );
      const password = this.configService.get<string>(
        XPRESSBEES_B2C_ENV_KEYS.AUTH_PASSWORD,
        'Mukesh@3724'
      );

      const url = `${baseUrl}${authPath}`;
      const payload: XpressbeesB2cAuthRequestDto = {
        email,
        password,
      };
      ...
      const response = await firstValueFrom(
        this.httpService.post<XpressbeesB2cAuthResponseDto>(url, payload, {
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      if (!response.data || !response.data.status || !response.data.data) {
        throw new CustomHttpException(
          ...
          'XpressBees B2C authentication failed: Invalid response'
        );
      }
      ...
      this.tokenExpiryTime = Date.now() + (3 * 60 * 60 * 1000) - XPRESSBEES_B2C_CONSTANTS.TOKEN_EXPIRY_BUFFER;

      this.logger.log('XpressBees B2C authentication successful');
      this.logger.debug(`Token will be refreshed at: ${new Date(this.tokenExpiryTime).toISOString()}`);

      return token;
    } catch (error) {
      this.logger.error('XpressBees B2C authentication failed', error.stack);
      this.token = null;
      ...
      throw new CustomHttpException(
        HttpStatus.UNAUTHORIZED,
        `XpressBees B2C authentication failed: ${error.message}`
      );
    }
  }
  ...
  invalidateToken(): void {
    this.logger.log('Invalidating XpressBees B2C token');
    this.token = null;
    this.tokenExpiryTime = null;
  }
}
