...
export class CustomHttpExceptionFilter implements ExceptionFilter {
  ...
  private async sendDiscordAlert(
    ...
    try {
      this.logger.log(`🚨 Sending Discord alert from CustomHttpExceptionFilter...`);
      ...
      this.logger.log(
        `📤 Sending Discord alert from CustomHttpExceptionFilter with payload: ${JSON.stringify(alertPayload, null, 2)}`
      );

      const result = await alertService.sendToDiscord(alertPayload);
      this.logger.log(
        `🔄 Discord API response from CustomHttpExceptionFilter: ${JSON.stringify(result)}`
      );

      this.logger.log(
        `✅ Discord alert sent successfully from CustomHttpExceptionFilter`
      );
    } catch (alertError) {
      this.logger.error(
        `❌ Failed to send Discord alert from CustomHttpExceptionFilter: ${alertError.message}`
      );
      this.logger.error(`Alert error stack: ${alertError.stack}`);
      this.logger.error(`Alert error details: ${JSON.stringify(alertError)}`);
    }
