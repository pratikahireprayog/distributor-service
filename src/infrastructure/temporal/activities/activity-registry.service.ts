...
export class ActivityRegistryService {
  ...
  private registeredActivities: Map<
    string,
  ...
  register(
    ...
    this.logger.log(
      `Registering activities for ${name}: ${Object.keys(activityMethods).join(", ")}`
    );
    // Ensure all methods are properly bound
    const boundMethods = Object.entries(activityMethods).reduce(
      (acc, [key, fn]) => {
